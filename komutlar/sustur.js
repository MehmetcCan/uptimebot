...
module.exports.run = async (bot, message, args) => {
  ...
  if(!tomute) return message.reply("Bu Komutu Kullanmak İçin : m!sustur <Kullanıcı> <Süre> Olarak Yazmalısınız.");
...
let muterole = message.guild.roles.find(r => r.name === "Cezalı");
  ...
  //end of create role
  let mutetime = args[1];
  if(!mutetime) return message.reply("Bu Komutu Kullanmak İçin : m!sustur <Kullanıcı> <Süre> Olarak Yazmalısınız.");
